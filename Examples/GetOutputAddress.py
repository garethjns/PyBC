...
def P2PKH(pk, 
    ...
    pk = pk.decode("hex")
    if debug: print "pk_op: {0}".format(pk.encode("hex"))
    
    # Add version
    pk = b"\00" + pk
    if debug: print "pk + ver: {0}".format(pk.encode("hex"))
    
    # Hash
    h1 = hashlib.sha256(pk).digest()
    if debug: print "h1: {0}".format(h1.encode("hex"))
    
    # Hash again
    h2 = hashlib.sha256(h1).digest()
    if debug: print "h2: {0}".format(h2.encode("hex"))
    
    # Add first 4 bytes of second hash to pk (already hex)
    pk = pk + h2[0:4]
    if debug: print "pk + checksum: {0}".format(pk.encode("hex"))
    
    # Convert to base 58 (bin -> base58)
    b58 = base58.b58encode(pk)
    if debug: print "b58: {0}".format(b58)
...
if debug: print "pk: {0}".format(pk.encode("hex"))

h1 = hashlib.sha256(pk).digest()
if debug: print "SHA256: h1: {0}".format(h1.encode("hex"))
# 600FFE422B4E00731A59557A5CCA46CC183944191006324A447BDB2D98D4B408

h2 = hashlib.new('ripemd160', h1).digest()
if debug: print "RIPEMD160 h2: {0}".format(h2.encode("hex"))
# 010966776006953D5567439E5E39F86A0D273BEE

# Add leading zeros
addr = b"\00" + h2
if debug: print "version + addr: {0}".format(addr.encode("hex"))
# 00010966776006953D5567439E5E39F86A0D273BEE

# Hash
# (Decode from hex, then back to hex for checking)
h3 = hashlib.sha256(addr).digest()
if debug: print "h3: {0}".format(h3.encode("hex"))
# 445C7A8007A93D8733188288BB320A8FE2DEBD2AE1B47F0F50BC10BAE845C094

# Hash again
# (h1 is already binary, then back to hex for checking)
h4 = hashlib.sha256(h1).digest()
if debug: print "h4: {0}".format(h4.encode("hex"))
# D61967F63C7DD183914A4AE452C9F6AD5D462CE3D277798075B107615C1A8A30

# Get checksum
cs = h4[0:4]
if debug: print "checksum: {0}".format(cs.encode("hex"))
# D61967F6

# Add checksum to second hash
addr = addr+cs
if debug: print "h4 + cs: {0}".format((addr).encode("hex"))
...
def PK2Addr(pk, 
    ...
    cs = h4[0:4]
    if debug: print "checksum: {0}".format(cs.encode("hex"))
    if debug: print "h4 + cs: {0}".format((h4 + cs).encode("hex"))
