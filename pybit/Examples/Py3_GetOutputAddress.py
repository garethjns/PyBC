...
h1 = hashlib.sha256(codecs.decode(pk, "hex")).digest()
# e158c4be10913422dadcf1c36843020ebb3ffe9d0cb13fb9e8c0a564a53c7832
print("h1: {0} | {1}".format(
        codecs.encode(h1, "hex"),
        "e158c4be10913422dadcf1c36843020ebb3ffe9d0cb13fb9e8c0a564a53c7832"))
...
print("h2: {0} | {1}".format(
        codecs.encode(h2, "hex"),
        "96bf1d277213bbcd91145138e4c7ad8dcd6e1de1c39884fcbc1f5a6d4d7aee93"))

cs = codecs.encode(h2[0:4], "hex")
print("checksum: {0} | {1}".format(cs,
                                   "96bf1d27"))

# Add first 4 bytes of second hash to pk (already hex)
pk = codecs.encode(codecs.decode(pk, "hex") + codecs.decode(cs, "hex"), 
                   "hex")
...
b58 = base58.b58encode(codecs.decode(pk, "hex"))
...
def P2PKH(pk,
    ...
    if debug:
        print("pk_op: {0}".format(codecs.encode(pk, "hex")))

    # Add version
    pk = b"\00" + pk
    if debug:
        print("pk + ver: {0}".format(codecs.encode(pk, "hex")))

    # Hash
    h1 = hashlib.sha256(pk).digest()
    if debug:
        print("h1: {0}".format(codecs.encode(h1, "hex")))

    # Hash again
    h2 = hashlib.sha256(h1).digest()
    if debug:
        print("h2: {0}".format(codecs.encode(h2, "hex")))

    # Add first 4 bytes of second hash to pk (already hex)
    pk = pk + h2[0:4]
    if debug:
        print("pk + checksum: {0}".format(codecs.encode(pk, "hex")))
...
assert addr == b"12higDjoCCNXSA95xZMWUdPvXNmkAduhWv"
...
if debug:
    print("pk: {0}".format(codecs.encode(pk, "hex")))

h1 = hashlib.sha256(pk).digest()
if debug:
    print("SHA256: h1: {0}".format(codecs.encode(h1, "hex")))
# 600FFE422B4E00731A59557A5CCA46CC183944191006324A447BDB2D98D4B408

h2 = hashlib.new('ripemd160', h1).digest()
if debug:
    print("RIPEMD160 h2: {0}".format(codecs.encode(h2, "hex")))
# 010966776006953D5567439E5E39F86A0D273BEE

# Add leading zeros
addr = b"\00" + h2
if debug:
    print("version + addr: {0}".format(codecs.encode(addr, "hex")))
...
if debug:
    print("h3: {0}".format(codecs.encode(h3, "hex")))
...
if debug:
    print("h4: {0}".format(codecs.encode(h4, "hex")))
# D61967F63C7DD183914A4AE452C9F6AD5D462CE3D277798075B107615C1A8A30

# Get checksum
cs = h4[0:4]
if debug:
    print("checksum: {0}".format(codecs.encode(cs, "hex")))
# D61967F6

# Add checksum to second hash
addr = addr+cs
if debug:
    print("h4 + cs: {0}".format(codecs.encode(addr, "hex")))
...
def PK2Addr(pk,
    ...
    if debug:
        print("checksum: {0}".format(codecs.encode(cs, "hex")))
        print("h4 + cs: {0}".format(codecs.encode((h4 + cs), "hex")))
...
assert addr == b"1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"
